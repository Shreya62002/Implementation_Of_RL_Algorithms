...
def policy_evaluation(P, nS, nA, policy, gamma=0.9, tol=1e-3):
    ...
    while True:
        ...
        for s in range(0,nS): 
            ...
            value_function[s]=value_function[s]+((P[s][policy[s]][0][2]+(gamma*P[s][policy[s]][0][0]*previous_value_function[P[s][policy[s]][0][1]])))
...
def policy_improvement(P, nS, nA, value_from_policy, policy, gamma=0.9):
    ...
    for s in range (0,nS):
        ...
        for a in range (0,nA):
            q[s][a]=q[s][a]+(P[s][a][0][2]+ (gamma*P[s][a][0][0]*value_from_policy[P[s][a][0][1]]))
...
def value_iteration(P, nS, nA, gamma=0.9, tol=1e-3):
    ...
    while error>tol and i<100:
        ...
        for s in range(0,nS):
                ...
                for a in range(0,nA):
                        q[s][a]=(P[s][a][0][2]+(gamma*P[s][a][0][0]*value_function[P[s][a][0][1]]))
    ...
    for s in range (0,nS):
        q1=np.zeros(nA)
        for a in range (0,nA):
            q1[a]=(P[s][a][0][2]+ (gamma*P[s][a][0][0]*value_function[P[s][a][0][1]]))
        new_policy=np.argmax(q1)
        policy[s]=new_policy
    return value_function, policy
