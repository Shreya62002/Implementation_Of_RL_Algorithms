...
def policy_evaluation(P, nS, nA, policy, gamma=0.9, tol=1e-3):
    ...
    while True:
        ...
        for s in range (0,nS): 
            ...
            #previous_value_function[s]=value_function[s]
            for i in range (0,len(P[s][policy[s]])):
                
                value_function[s]=value_function[s]+((P[s][policy[s]][i][2]+(gamma*P[s][policy[s]][i][0]*previous_value_function[P[s][policy[s]][i][1]])))
        ...
        if(max(np.abs(value_function-previous_value_function))<tol):
            return value_function
            
        previous_value_function=value_function
...
def policy_improvement(P, nS, nA, value_from_policy, policy, gamma=0.9):
    ...
    for s in range (0,nS):
        ...
        for a in range (0,nA):
            for i in range (0,len(P[s][policy[s]])):
                q[s][a]=q[s][a]+(P[s][a][i][2]+ (gamma*P[s][a][i][0]*value_from_policy[P[s][a][i][1]]))
...
def policy_iteration(P, nS, nA, gamma=0.9, tol=10e-3):
    ...
    while i<100:
        ...
        policy_diff=(new_policy-policy)
        if max(policy_diff)==0 :
...
def value_iteration(P, nS, nA, gamma=0.9, tol=1e-3):
    ...
    while error>tol and i<100:
        ...
        for s in range(0,nS):
                q = np.zeros((nS,nA))
                for a in range(0,nA):
                    for j in range (0,len(P[s][a])):

                        q[s][a]+=(P[s][a][j][2]+P[s][a][j][0]*(gamma*value_function[P[s][a][j][1]]))
                new_value_function[s]=max(q[s])
        error = max(error,max(np.abs(new_value_function-value_function)))
    ...
    for s in range (0,nS):
        ...
        for a in range (0,nA):
            ...
            for k in range (0,len(P[s][a])):
                q1[s][a]+=P[s][a][k][0]*(P[s][a][k][2]+ (gamma*value_function[P[s][a][k][1]]))
        new_policy=np.argmax(q1[s])
        policy[s]=new_policy
...
if __name__ == "__main__":
    ...
    env = gym.make(name,is_slippery=True)
    print("\n" + "-" * 25 + "\nBeginning Policy Iteration\n" + "-" * 25)

    V_pi, p_pi = policy_iteration(P, nS, nA, gamma=0.9, tol=1e-3)
    episode_reward =render_single(env, p_pi, 100)
    print("\n" + "-" * 25 + "\nBeginning Value Iteration\n" + "-" * 25)

    V_vi, p_vi = value_iteration(P, nS, nA, gamma=0.9, tol=1e-3)
    
    episode_reward1=render_single(env, p_vi, 100)
